...
function Route:init(config_file)
	...
	-- 初始化路由配置
	self.config_route = config["route"] or {} --全匹配
	self.config_route_pattern = config["route_pattern"] or {} --模式匹配
...
function Route:getRoute(uri, method)
	...
	for i, val in ipairs(self.config_route) do
		if (val[1] == "*" or string.upper(val[1]) == string.upper(method)) and (uri == val[2] or uri == (val[2] .. "/")) then
			return { class = val[3], method = val[4] }
		end
	end
	...
	for i, val in ipairs(self.config_route_pattern) do
		...
		if is ~= nil and (val[1] == "*" or string.upper(val[1]) == string.upper(method)) then
			return { class = val[3], method = val[4] }
		end
	end
...
--[[
拦截器配置，注：拦截器按需实现beforeHandle和afterHandle方法
interceptor = {
    {
        url = {
